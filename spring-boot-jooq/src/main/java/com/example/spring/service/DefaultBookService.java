...
public class DefaultBookService implements BookService
{
    ...
    public void create(int id, int authorId, String title)
    {
        ...
        // should lead to a constraint violation, which should roll back the whole
        // transaction
        for (int i = 0; i < 2; i++)
            dsl.insertInto(BOOK).set(BOOK.ID, id).set(BOOK.AUTHOR_ID, authorId)
                    .set(BOOK.TITLE, title).execute();
    }
