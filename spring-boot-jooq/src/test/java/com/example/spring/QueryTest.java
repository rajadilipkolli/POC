...
public class QueryTest
{
    ...
    public void testJoin() throws Exception
    {
        ...
        Result<Record3<String, String, Integer>> result = create
                .select(a.FIRST_NAME, a.LAST_NAME, DSL.countDistinct(s.NAME)).from(a)
                .join(b).on(b.AUTHOR_ID.equal(a.ID)).join(t).on(t.BOOK_ID.equal(b.ID))
                .join(s).on(t.BOOK_STORE_NAME.equal(s.NAME))
                .groupBy(a.FIRST_NAME, a.LAST_NAME)
                .orderBy(DSL.countDistinct(s.NAME).desc()).fetch();
        ...
        assertEquals(Integer.valueOf(3), result.getValue(0, DSL.countDistinct(s.NAME)));
        assertEquals(Integer.valueOf(2), result.getValue(1, DSL.countDistinct(s.NAME)));
    }
