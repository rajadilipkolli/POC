...
describe('WelcomeComponent', () => {
  ...
  beforeEach(waitForAsync(() => {
    const welcomeDataServiceSpy = jasmine.createSpyObj('WelcomeDataService', 
      ['executeHelloWorldBeanServiceWithPathVariable', 'executeHelloWorldBeanService']);
    ...
    TestBed.configureTestingModule({
      ...
      .then(() => {
        ...
        welcomeDataService = TestBed.inject(WelcomeDataService) as jasmine.SpyObj<WelcomeDataService>;
  ...
  it('should handle successful welcome message with path variable', () => {
    ...
    welcomeDataService.executeHelloWorldBeanServiceWithPathVariable.and.returnValue(of(mockResponse));

    component.getWelcomeMessage();

    expect(welcomeDataService.executeHelloWorldBeanServiceWithPathVariable).toHaveBeenCalledWith('testUser');
  ...
  it('should fallback to basic service when path variable service fails', () => {
    ...
    welcomeDataService.executeHelloWorldBeanServiceWithPathVariable.and.returnValue(throwError(() => pathVariableError));
    ...
    expect(console.log).toHaveBeenCalledWith('Error with path variable, falling back to basic service:', pathVariableError);
  });
  ...
  it('should handle error when both services fail', () => {
    ...
    welcomeDataService.executeHelloWorldBeanServiceWithPathVariable.and.returnValue(throwError(() => pathVariableError));
    welcomeDataService.executeHelloWorldBeanService.and.returnValue(throwError(() => basicServiceError));
    ...
    expect(welcomeDataService.executeHelloWorldBeanServiceWithPathVariable).toHaveBeenCalledWith('testUser');
    expect(welcomeDataService.executeHelloWorldBeanService).toHaveBeenCalled();
    expect(component.welcomeMessage).toBe('An unexpected error occurred. Please check if the backend server is running.');
    expect(console.error).toHaveBeenCalledWith('Error fetching welcome message:', basicServiceError);
  });
  ...
  it('should handle HTTP error response with message', () => {
    ...
    welcomeDataService.executeHelloWorldBeanServiceWithPathVariable.and.returnValue(throwError(() => httpError));
