...
public class MultiplyTwoBigString {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "Multiply Brute Force Way -> " + multiply(a, b) + ", TimeTaken -> " + (
            System.nanoTime() - startTime));
    startTime = System.nanoTime();
    System.out.println(
        "Multiply Optimized Way -> " + multiplyOptimized(a, b) + ", TimeTaken -> " + (
            System.nanoTime() - startTime));
    startTime = System.nanoTime();
    System.out.println(
        "Multiply MostOptimized Way -> " + multiplyMostOptimized(a, b) + ", TimeTaken -> "
            + (System.nanoTime() - startTime));
  ...
  private static String multiply(String firstStringArray, String secondStringArray) {
    ...
    int[][] tempArray = new int[firstStringArray.length()][secondStringArray.length()
        + firstStringArray.length()];

    for (int i = firstStringArray.length(); i > 0; i--) {
      int tempValueToAdd = 0;
      int arrayRowIndex = firstStringArray.length() - i;
      int arrayColumnIndex = (firstStringArray.length() + secondStringArray.length()) - (
