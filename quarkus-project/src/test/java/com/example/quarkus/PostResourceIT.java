...
public class PostResourceIT {
    ...
    void getNoneExistedPostShouldReturn404() {
        given().when()
                .get("/posts/999999") // Use a non-existent numeric ID
                .then()
                .statusCode(404);
    }
    ...
    void deletePostShouldRemovePost() {
        ...
        // Verify it's gone
        given().when().get("/posts/" + id).then().statusCode(404); // Should return Not Found after deletion
    }
    ...
    void getAllPostsShouldReturnListOfPosts() {
        ...
        // Get all posts
        given().when().get("/posts").then().statusCode(200).contentType(ContentType.JSON);
    }
    ...
    void deleteNonExistentPostShouldReturn404() {
        given().when().delete("/posts/999999").then().statusCode(404);
    }
}
